...
<%@ page import="cn.sut.order.gt.vo.PendingAccount" %>
...
{
var t = setTimeout(function(){window.location.reload();},10000);
 
}
function orderinfo(orderId){//初始化数据
	$.ajax({
        type: "POST",
        url: "/order/OrderDishesServlet",
        data: {
       	 orderId : orderId,
        },
        dataType: "json",
...
function updateinfo(orderId){
	like=window.confirm("确认收到买单款项？");
	if(like==true){
		$.ajax({
	        type: "POST",
	        url: "/order/ReceivableServlet",
	        data: {
	       	 orderId : orderId,
	        },
	        dataType: "json",
			...
			<ul class="nav navbar-nav navbar-right">
				...
				<li class="dropdown"><a href="#" class="dropdown-toggle"
					...
					aria-expanded="false"><img
						...
						class="img-circle" style="border:1px solid #FFF" />&nbsp;&nbsp;当前用户:【${user.userAccount}】,身份为管理员 <span class="caret"></span></a>
		...
		<div class="row">
			<div class="col-sm-3 col-md-2 sidebar">
				<ul class="nav nav-sidebar">
					...
					<li class="active"><a href="OrderInfoServlet"><i
			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
				...
				<div class="panel panel-danger">
					...
					<div class="panel-body" style="padding-bottom: 0px">
						...
						<c:forEach items="${ requestScope.lists1}" var = "list1" begin='0' end='3'>
						<div class="col-xs-6 col-sm-3 placeholder">
								<img data-src="holder.js/200x200/auto/vine"
									...
									src="${list1.deshesImg }">
								<h4>${list1.dishesName}</h4>
								<span class="text-muted">${list1.dishesDiscript }</span>
							</div>
						</c:forEach>
						...
						<ul class="pager" style="margin-right: 20px">
							...
							<li class="next"><a href="pages/admin/dishesadmin.jsp">更多特色菜品 <span
							...
							<table class="table table-striped">
								...
								<tbody id="orderTable">
									...
									<c:forEach items="${ requestScope.lists}" var = "list">
										<td>${list.tableId }</td>
										<td>${list.orderBeginDate }</td>
										<td>${list.orderEndDate }</td>
										<td>${list.total }</td>
										<td><i style="cursor: pointer; font-size: 14;"
											onmouseover="this.style.color='orange'"
											onmouseout="this.style.color='black'"
											class="icon-credit-card icon-large" title="确认结账"
											onclick="updateinfo(${list.orderId})"></i>&nbsp;&nbsp;<i
											style="cursor: pointer; font-size: 14;"
											onmouseover="this.style.color='orange'"
											onmouseout="this.style.color='black'"
											class="icon-sitemap icon-large" title="查看订单详情"
											onclick="orderinfo(${list.orderId})"></i> &nbsp;&nbsp;<i
											style="cursor: pointer; font-size: 14;"
											onmouseover="this.style.color='orange'"
											onmouseout="this.style.color='black'"
											class=" icon-remove-sign icon-large" title="订单作废"
											onclick="deleteinfo(${list.orderId})"></i></td>
									</tr>
									</c:forEach>
