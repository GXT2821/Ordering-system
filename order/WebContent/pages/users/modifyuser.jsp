...
function FileUpload() {
	...
	function userCallback() {
		...
		if(userOldPass==""){
    		...
    		$("#usererror").html("旧密码不能为空！");
		}else{
			$.ajax({
				 type: "POST",
			        url: "/order/OldPassServlet",
			        data: {
			        	userId : userId,
			        ...
			        success: function (json) {
			        	if(json.msg=="OK"){
			        		...
			        		$("#usererror").html("您输入的原密码错误，请重新输入！");
			        	}
	...
	function check(){
		...
		var userId = $("#userId").val();
		var faceimg = $("#faceimgname").val();
		$.ajax({
			 type: "POST",
		        url: "/order/UpdatePasswordServlet",
		        data: {
		        	userId : userId,
		        	userPass : userPass,
		        	faceimg : faceimg,
...
<body style="font-family:微软雅黑;background-image: url('img/body-bg.png')">
	...
	<div class="jumbotron"
		...
		<div class="container">
			...
			<p>
				亲爱的<span style="color: red;font-weight: bold;">【${user.userAccount}】</span>,${ORDER_SYS_NAME}提示：您可以在本界面修改您的账户密码及头像信息！
			</p>
		...
		<div class="row" style="padding-top: 0px">
			<div style="width: 50%;display: inline-block;">
				<div>
					...
					<form class="form-horizontal" role="form" style="margin-top: 20px"
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<select class="form-control" name="roleId" disabled="disabled">
									<option value="1"
										<c:if test="${user.role==1}"> selected="selected"</c:if>>餐厅管理员</option>
									<option value="2"
										<c:if test="${user.role==2}"> selected="selected"</c:if>>厨师</option>
									<option value="3"
										<c:if test="${user.role==3}"> selected="selected"</c:if>>点餐员</option>
						...
						<input type="hidden" value="${user.faceimg}" id="faceimgname"
							name="faceimg" />

						<div class="form-group">
							<div class="col-sm-offset-2 col-sm-10">
								<input type="button" class="btn btn-danger" disabled="disabled"
									id="addbtu" value="确认修改" onclick="check()"/>
							</div>
						</div>
					</form>
				</div>
			...
			<div style="width: 40%;display: inline-block;vertical-align: top;">
				...
				<div style="text-align: center;">
					<img src="${user.faceimg}" id="face" width="200px"
